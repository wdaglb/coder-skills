# UPGRADE

记录已发布版本的主要变化，便于安装、升级与回溯。

## 1.0.20 - 2026-07-16

移除收尾风险审查技能，并将默认验证口径统一收敛为静态 lint。

### 移除

- 删除 `audit-completion-risks` 技能及其 Agent 元数据。
- `execution-gate` 与 `plan-implementation-tasks` 不再把收尾风险审查作为任务或最终验证的前置条件。

### 变更

- 所有任务默认只运行当前项目已有的静态 lint：
  - 不再因风险等级、GitNexus 影响面或任务类型自动追加类型检查、构建、测试、smoke、浏览器或手工验证。
  - 只有用户明确指定测试时，才生成或执行对应测试；lint 不可用时明确报告验证未完成，不以其他检查替代。
- 调整 `pencil-design-replica` 与 `web-design`：
  - 浏览器预览、截图、响应式和交互验证改为用户明确指定时才执行。
- 移除发布 workflow 的 `npm pack --dry-run` 打包预检，保留包元数据与 tag 版本一致性校验。

### 影响

- 这是一个行为变更版本；CLI 用法、安装路径与包结构保持不变。
- 使用旧版收尾审查或自动升级测试流程的协作方式需要调整为显式指定测试。

## 1.0.19 - 2026-07-12

收紧显式失败处理与任务执行边界，新增网页设计规范，并补充 GitNexus 代码理解约束。

### 新增

- 新增 `web-design` 技能：
  - 在页面布局和表单承载方式未明确时，要求先收敛自适应与页面/弹窗/抽屉边界
  - 规范复杂字段的业务说明、用户可见文案和浏览器真实渲染检查
- 新增仓库内 GitNexus 使用说明与配套技能入口：
  - 修改符号前先评估影响面，提交前检查变更所影响的符号和执行流程

### 变更

- 调整 `src/DOC.md`：
  - 明确禁止隐式兜底、静默降级和无依据兼容分支，要求以可观测的显式失败替代
  - 收紧前端用户文案边界，禁止泄露实现、调试或助手生成过程
  - 明确简单任务优先按最短可验证闭环执行，不机械拆分并行任务或测试流水线
- 调整 `ambiguity-planner`、`execution-gate` 与 `plan-implementation-tasks`：
  - 方案收敛后直接写入正式方案文件，并按方案真实复杂度选择拆分或直接实施
  - 简单方案优先使用少量高价值任务收口；高影响代码改动才提升验证与风险审查强度
- 同步更新 `audit-completion-risks`、`pencil-design-replica` 及对应 Agent 提示词，统一新的执行与交付边界。

### 影响

- 这是一个向后兼容的补充版本；CLI 用法、安装路径与包结构保持不变。
- 后续任务会更明确地区分真实失败与兜底行为，规划和实施流程也会根据实际复杂度收敛，避免形式化拆分。

## 1.0.18 - 2026-06-18

收紧方案确认链路、实施任务测试边界，并补充分层目录规范。

### 变更

- 调整 `ambiguity-planner` 与 `execution-gate`：
  - 边界询问的推荐方案固定为第 1 个选项，并在选项后内联推荐理由
  - `【选择】` 固定在回复底部，不再单独输出 `【当前推荐】` 段落
  - 方案收敛后直接生成预览草稿文件，并通过一次“方案文件审查”完成确认写入
  - 方案文件落地后固定询问是否拆分实施任务；用户选择“不拆分，直接进入开始实施”时不再二次确认
- 调整 `plan-implementation-tasks`：
  - 子智能体负责的编码任务只做代码静态验证
  - 最终自动化测试、集成测试、端到端测试、手工验收或回归测试必须拆为独立测试任务
  - 进入最终测试前必须先使用 `audit-completion-risks` 审查当前 diff 风险
- 调整 `src/DOC.md`：
  - 新增简化的前后端分层目录规范
  - 明确具体文件名、类名是否追加分层后缀由当前项目决定
  - 补充常见可选分层，并说明文件过多时可按业务域增加子目录

### 影响

- 这是一个向后兼容的补充版本；CLI 用法、安装路径与包结构保持不变。
- 重任务规划、方案文件写入、实施任务拆分和最终测试前风险审查的边界更清晰；项目协作规范更容易按前后端分层落地，但不会强制套用固定目录模板。

## 1.0.17 - 2026-06-09

强化前后端项目实施任务拆分规则，避免把并发拆分机械固定成 5 个任务。

### 变更

- 调整 `plan-implementation-tasks`：
  - 前后端项目默认先串行规划或补齐接口约定文档
  - 前端编码、后端编码、联调与集成测试任务按实际功能域、模块边界、文件重叠和依赖关系扩展
  - 明确前后端并发拆分不是固定 5 个任务，也不是固定 5 个分组
  - 禁止跳过接口契约确认后直接让前后端并行编码
  - 其它项目类型不套用前后端依赖骨架，继续按真实依赖分析并行性

### 影响

- 这是一个向后兼容的补充版本；CLI 用法、安装路径与包结构保持不变。
- 前后端项目拆分任务时会更稳定地先收敛接口契约，再按实际模块扩展可并行任务，减少机械拆分和提前联调 / 测试的问题。

## 1.0.16 - 2026-06-08

收紧执行门禁、重任务规划回复格式和前端 UI 文案边界，并补齐收尾风险审查技能。

### 新增

- 新增 `audit-completion-risks` 技能：
  - 用于代码任务完成后的当前 diff 风险审查
  - 只列真实风险，按严重程度排序，并给出文件位置
  - 避免在最终交付前遗漏回归风险、失败路径、无关改动或验证不足

### 变更

- 调整 `execution-gate`：
  - 扩展触发描述，覆盖开始、继续、实现、修复、修改、新增、补充、生成、写入、执行、落地、排查并处理等实质执行请求
  - 首段回复固定为 `【任务理解】`、`【判断】`、`【风险】`、`【做法】/【下一步】`
  - 不再使用 `【实施中】`、`【规划确认中】` 等容易跑偏的阶段标题
- 调整 `ambiguity-planner`：
  - 强制分步收敛回复使用 `【当前步骤】`、`【选择】`、`【当前推荐】` 的固定结构
  - 方案写入审查改为先生成可点击预览草稿文件，再由用户确认写入正式 `docs/plans` 文件
  - 不再把完整方案正文打印到对话里，避免确认选项被 Markdown 代码块包住
- 调整 `src/DOC.md`：
  - 新增前端 / UI 文案生成边界
  - 禁止把提示词残留、实现说明、调试说明或内部技术细节写入用户可见页面

### 影响

- 这是一个向后兼容的补充版本；CLI 用法、安装路径与包结构保持不变。
- 实质执行请求更容易触发门禁，重任务规划输出更稳定，方案确认链路更适合在长 Markdown 方案中预览和确认。

## 1.0.15 - 2026-06-04

强化方案落地、子智能体并行实施和协作规范的触发稳定性。

### 变更

- 调整 `ambiguity-planner` 与 `execution-gate`：
  - 方案文件准备写入 `docs/plans` 前，必须先输出完整方案内容和目标路径让用户审查
  - 只有用户选择 `1. 确认写入“方案文件路径”` 时才允许写入；非 `1` 输入默认视为修改方案，并继续审查
  - 缺少完整方案正文时，不得编造方案、输出空方案或要求用户确认写入
  - 方案写入后退出方案规划阶段，方案文件默认只读，再进入是否拆分实施任务确认
- 调整 `plan-implementation-tasks`：
  - 将“多 agent”执行语义收敛为“主智能体 / 子智能体（sub-agent）”
  - 明确子智能体并行进入条件：写入范围不重叠、依赖性不大、验证可独立完成、前置依赖已完成
  - 明确主智能体自动负责子智能体派发、完成度监测、diff 审查、整合验证和下游任务推进
  - 实施确认选项区分“启用子智能体并行执行”和“主智能体串行执行”
- 调整 `src/DOC.md`：
  - 将编码原则、注释规范、技术内容、Git 提交、输出风格和禁止事项拆分为更容易触发的小分组
  - 新增“不猜、不假、不装”原则
  - 明确非 Git 仓库协作目录不应每次机械执行 Git 检查
  - 强化同事式协作语气，避免模板化 AI 口吻

### 影响

- 这是一个向后兼容的补充版本；CLI 用法、安装路径与包结构保持不变。
- 重任务规划、方案文件写入和实施任务并行执行的授权边界更清晰；文档规则分组后更容易被模型触发。

## 1.0.14 - 2026-06-01

补充仓库级协作规范，收紧文档、修复与 Git 操作边界。

### 变更

- 调整 `src/DOC.md`：
  - 明确代码修改完成后，不能因为上下文历史中出现过“提交代码”“commit”等表述就自动执行 Git 写操作
  - 只有用户当前最后一句回复明确要求“提交代码”时，才允许进入 Git 提交流程
  - 明确不要用“最小实现”“只跑通当前场景”替代完整交付，常见失败与边界场景需要按风险补齐兜底逻辑
  - 明确修复 bug 前必须先尝试复现问题，记录复现路径、输入条件、实际结果与期望结果后再进入修复
  - 明确仅创建或修改文档时，不运行测试、构建、lint 等代码验证命令，只审查一遍文档内容即可

### 影响

- 这是一个向后兼容的补充版本；CLI 用法、安装路径与包结构保持不变。
- Git 写操作授权边界更严格，文档类任务的验证路径更轻量，bug 修复流程更强调先复现再修复。

## 1.0.13 - 2026-05-31

锁定实施阶段的方案与任务文件，避免执行中反向改写已确认计划。

### 变更

- 调整 `plan-implementation-tasks`：
  - 用户确认按 `implementation-guide.md` 按步骤执行后，`docs/plans/**`、`docs/tasks/**` 和 `implementation-guide.md` 默认只读
  - 多 agent 派发模板将方案、任务与实施指导文件列为禁止修改范围
  - 发现任务文件与代码现状冲突时，默认停止并回到主 agent 协调，不直接改写任务文件
- 调整 `ambiguity-planner` 与 `execution-gate`：
  - 明确实施开始后不得修改已落地的方案文件和任务文件
  - 只有用户明确要求修改时，才允许回到方案或任务文件调整

### 影响

- 这是一个向后兼容的补充版本；CLI 用法、安装路径与包结构保持不变。
- 实施阶段会更严格地区分“执行输入”和“实施输出”，防止 agent 为了继续执行而擅自改写已确认计划。

## 1.0.12 - 2026-05-31

新增实施任务规划技能，并收紧方案落地后的执行确认链路。

### 新增

- 新增 `plan-implementation-tasks` 技能：
  - 将已确认的 `docs/plans` 方案拆分为 `docs/tasks/<方案文件>/tasks-N.md`
  - 每个实施步骤单独生成一个任务文件
  - 同步生成 `implementation-guide.md`，明确串行 / 并行分组、多 agent 执行建议、详细执行流程与边界事件处理

### 变更

- 调整 `ambiguity-planner` 与 `execution-gate`：
  - 方案文件落地后先确认是否拆分实施任务
  - 用户确认拆分时调用 `plan-implementation-tasks`
  - 实施任务规划完毕后，再确认是否按 `implementation-guide.md` 按步骤执行
- 将技能内固定选项格式从 `A / B / C / D` 统一改为 `1 / 2 / 3 / 4`
- 同步更新 `git-action` 的脏工作树确认模板，保持固定选项格式一致

### 影响

- 这是一个向后兼容的补充版本；CLI 用法、安装路径与包结构保持不变。
- 方案落地后的执行链路更严格：写入方案、拆分任务、按实施指导执行分别是独立确认点。
- 后续涉及可并行实施的任务，会优先通过 `implementation-guide.md` 明确多 agent 分工与停手边界。

## 1.0.11 - 2026-05-21

收敛 README 定位、安装覆盖策略与 DOC 协作规范。

### 变更

- 重写 `README.md`，将重点收敛为 skills 的作用、价值与内置能力，弱化安装脚本本身的介绍占比。
- 调整 `install` 默认行为：当目标已存在且不是当前仓库同一软链接时，默认直接覆盖安装，不再区分交互/非交互终端确认。
- 调整 `src/DOC.md` 协作规范：
  - 新增“始终提供完整、健壮、可扩展的解决方案，而非仅满足当前需求的最小实现”
  - 新增“助手回复应该更自然，更像一位同事”
  - 新增“需要用户确认时，禁止使用开放式回答句式；必须优先提供固定选项”
  - 新增“废弃旧方法不应直接删除；可先将整个旧方法注释保留，并注明废弃时间与原因”

### 影响

- 这是一个向后兼容的补充版本；外部安装入口与基本命令保持不变。
- `install` 会更强势地覆盖旧目标；若本地已有手工文件或自建链接需要保留，安装前应先自行备份。
- README 会更强调 skills 本身，而不是把仓库理解成单纯的安装工具。

## 1.0.10 - 2026-05-17

安装命令新增 OpenCode 目标，便于把规则与 skills 一起挂到 OpenCode 本地目录。

### 新增

- `install` 子命令新增 `opencode` 目标：
  - 支持 `--target opencode`
  - `--target all` 现在会同时覆盖 `codex`、`claude`、`opencode`
- 新增环境变量 `OPENCODE_HOME`，默认路径为 `~/.config/opencode`
- OpenCode 安装路径为：
  - `~/.config/opencode/AGENTS.md`
  - `~/.config/opencode/skills/<skill-name>`

### 文档说明

- `README.md` 已同步补充 OpenCode 支持、示例命令与自定义目录说明。
- 同步补充通用 Git 提交规范，明确 commit message 不得追加 AI 助手身份尾巴。
- `README.md` 已同步更新 install 覆盖策略说明：已存在目标默认直接覆盖。

### 影响

- 这是一个向后兼容的补充版本；原有 `codex` / `claude` 安装方式不变。
- 若你此前对 OpenCode 采用手工拷贝或自建链接，改用新版 `install` 时会被默认替换为当前仓库软链接；如需保留旧内容，需先自行备份。
- install 子命令的覆盖行为已改为：只要目标已存在且不是当前仓库同一软链接，就默认直接替换，不再区分交互/非交互终端确认。

## 1.0.9 - 2026-04-26

强化重歧义任务的主动补盲要求，并同步调整执行门禁的交付摘要措辞。

### 变更

- 调整 `execution-gate`：
  - 明确重歧义任务需要主动识别用户未显式提出、但会影响方案稳定性的边界、异常与取舍
  - 要求转交 `ambiguity-planner` 时主动补盲，将范围、权限、数据、兼容、失败、回滚、验证与发布等关键点压成固定选项
  - 将紧凑交付摘要中的 `下一步` 字段改为 `建议`，避免把可选建议误读为必须动作
- 同步更新 `execution-gate` 的 OpenAI agent 默认提示词，确保入口文案与新规则一致

### 影响

- 这是一个向后兼容的发布版本；CLI 用法、安装路径与包结构保持不变
- 技能在重歧义任务中会更主动补齐用户未提及但会影响决策的关键边界

## 1.0.8 - 2026-04-24

收紧规划与交付口径，避免开放式补需求和过长交付摘要。

### 变更

- 调整 `ambiguity-planner`：
  - 明确需求未收清时继续问到可稳定实施
  - 明确简单事项不应被硬拆成多轮收敛
  - 补充输出验收清单与手工前向样例约束
- 调整 `execution-gate`：
  - 明确关键边界未闭合时不得降级放行
  - 明确已足够实施的小问题不得升级成重歧义
  - 新增紧凑版交付摘要格式，默认不单列“改动”区块

### 影响

- 这是一个向后兼容的发布版本；CLI 用法与安装路径保持不变
- 技能输出会更偏向固定选项收敛与短版交付摘要
## 1.0.7 - 2026-04-23

完善协作规范文案，并收敛规划与 Git 技能的执行边界。

### 变更

- 重写 `README.md`，补充仓库定位、适用场景、安装结果、覆盖规则与目录结构说明
- 调整 `src/DOC.md`：
  - 统一改为“AI 助手”表述
  - 移除仓库内多代理派发策略说明
  - 收敛交付示例，避免把过程性进度模板混入最终交付格式
- 重构 `execution-gate`：
  - 从“低 / 中 / 高歧义”收敛为“轻 / 重歧义”两级判定
  - 明确轻歧义任务可直接执行，重歧义任务必须转交 `ambiguity-planner`
  - 强化固定开场、真实状态一致与高风险边界升级规则
- 调整 `ambiguity-planner`：
  - 要求每轮给出当前估计总步数
  - 用户选择前不再单独输出“影响”分节
  - 影响说明改为在边界确认后进入下一步时补充
- 调整 `git-action`：
  - 明确提交消息必须概括实际功能改动
  - 补充空泛 commit message 的禁止规则
- 同步更新相关 `agents/openai.yaml` 提示词，确保技能入口文案与规则一致

### 影响

- 这是一个向后兼容的发布版本；CLI 安装路径与基本使用方式保持不变
- 依赖旧版技能文案行为的协作习惯，需要按新的轻 / 重歧义与分步收敛规则适配

## 1.0.6 - 2026-04-22

重构技能目录布局，并强化规划类技能的分工与收敛规则。

### 变更

- 将仓库内技能源目录迁移到 `src/` 下：
  - `AGENTS.md` / `skills/*` 的源布局调整为 `src/DOC.md` 与 `src/skills/*`
  - CLI 安装逻辑同步改为从 `src/` 读取源文件
- 保持外部安装目标不变：
  - Codex 仍安装到 `~/.codex/AGENTS.md` 与 `~/.codex/skills/*`
  - Claude Code 仍安装到 `~/.claude/AGENT.md` 与 `~/.claude/skills/*`
- 新增 `ambiguity-planner` 技能，用于处理中 / 高歧义任务的分步收敛与逐步规划
- 重构 `execution-gate`，将其职责收敛为统一门禁、低歧义简化规划、中高歧义转交与边界把关
- 明确中 / 高歧义任务的方案文件规则：
  - 仅中 / 高歧义任务询问是否落实到方案文件
  - 用户确认后，自动检查并创建 `docs/plans` 后再写入方案文件
- 强化 `ambiguity-planner` 的推进边界：
  - 允许持续推进
  - 防止无限推进
  - 防止过深推进到实现级细节

### 影响

- 这是一个向后兼容的发布版本；外部安装路径与 CLI 基本用法保持不变
- 若有直接依赖仓库内部源路径的脚本，需要从根目录 `AGENTS.md` / `skills/*` 切换到 `src/DOC.md` / `src/skills/*`

## 1.0.2 - 2026-04-19

补充 `large-file-gate` 技能，并明确 Claude Code 侧软链接行为。

### 新增

- 新增 `large-file-gate` 技能，用于在读取未知大小的文件前先执行 100KB 门禁检查，避免把大日志、大导出文件或生成产物直接读入上下文。
- `large-file-gate` 会随 `install` 一起安装到：
  - Codex：`~/.codex/skills/large-file-gate`
  - Claude Code：`~/.claude/skills/large-file-gate`

### 文档说明

- 明确了 Claude Code 目标文件名为单数 `AGENT.md`，目标路径为 `~/.claude/AGENT.md`。
- 补充了覆盖规则说明：当 `~/.claude/AGENT.md` 已存在且不是当前项目软链接时，安装流程不会静默覆盖，交互模式下会询问确认，非交互模式下需显式传入 `--force`。

### 影响

- 这是一个向后兼容的补充版本；现有 CLI 用法保持不变。
- 新版本安装后，Codex / Claude Code 均可额外获得 `large-file-gate` 技能目录软链接。

## 1.0.1 - 2026-04-19

修正发布配置并切换到 scoped npm 包名。

### 变更

- npm 包名从 `coder-skills` 调整为 `@kingeast/coder-skills`
- CLI 命令名保持不变，仍然使用 `coder-skills`
- `README.md` 中涉及安装包名的命令已同步改为 scoped 包名：
  - `pnpm add -D @kingeast/coder-skills`
  - `pnpm dlx @kingeast/coder-skills install`
- GitHub Actions 发布流程已升级到 Node 24 兼容写法，避免 Node 20 弃用告警影响后续发布

### 影响

- 之后发布到 npm 的目标包名为 `@kingeast/coder-skills`
- `pnpm exec coder-skills install` 与本地仓库内的 `pnpm coder-skills install` 用法不变

## 1.0.0 - 2026-04-19

首个可发布版本。

### 新增

- 新增 npm 包 `@kingeast/coder-skills`，其 CLI 命令为 `coder-skills`，可执行安装命令：
  - `pnpm coder-skills install`
  - `pnpm exec coder-skills install`
  - `pnpm dlx @kingeast/coder-skills install`
- 新增 `install` 子命令，支持将当前项目 `src/` 目录下的 `DOC.md` 与 `skills/*` 以软链接方式安装到：
  - Codex：`~/.codex/AGENTS.md`、`~/.codex/skills/<skill-name>`
  - Claude Code：`~/.claude/AGENT.md`、`~/.claude/skills/<skill-name>`
- 新增常用参数：
  - `--target codex`
  - `--target claude`
  - `--force`

### 行为说明

- 安装时会逐个 skill 建立软链接，不会整体替换目标端整个 `skills` 目录。
- 如果目标已经是当前项目的同一软链接，则会直接跳过，避免重复删除重建。
- 如果目标已存在但不是当前链接：
  - 交互终端会询问是否覆盖
  - 非交互终端不会自动覆盖；如需覆盖需显式传入 `--force`

### 文档与发布

- 新增 `README.md`，提供最小可用命令说明。
- 新增 GitHub Actions 发布流程：
  - 推送 `v*` tag 时自动校验并发布到 npm
  - 支持 `workflow_dispatch` 手动触发发布
- 补齐 npm 发布所需的基础元数据，包括：
  - `description`
  - `files`
  - `keywords`
  - `repository`
  - `homepage`
  - `bugs`
  - `engines`
  - `publishConfig`
