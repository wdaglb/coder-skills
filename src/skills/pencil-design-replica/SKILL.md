...
落地完成后默认只完成以下验证：
- [ ] 运行当前项目已有的静态 lint
- [ ] lint 缺失或无法运行时，明确记录验证未完成，不以类型检查、构建、测试或运行态验证替代
- [ ] 若存在 GitNexus 影响分析能力，可用于说明影响范围，但不得自动追加测试、smoke 或运行态验证
- [ ] 输出交付说明，明确“仅运行静态 lint，未运行测试或运行态验证”

运行项目、截图比对、展示/边界情况检查和原有业务功能回归仅在用户明确指定时执行；改动风险或 GitNexus 影响面不得自动改变这一验证口径。
