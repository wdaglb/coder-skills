...
落地完成后需完成以下验证：
- [ ] 简单静态还原或局部样式/文案调整默认只做 lint、类型检查、格式检查、静态审查或构建期静态检查；不要主动跑完整测试、smoke、启动项目或手工回归
- [ ] 若当前技能列表或工具列表中存在 GitNexus 影响分析能力，且 UI 改动影响真实组件、路由、接口绑定或共享状态，应先判断影响面；影响达到 MEDIUM/HIGH/CRITICAL、多流程、关键路径或 d=1 调用方较多时，需要补必要测试、smoke 或运行态验证
- [ ] 运行项目，截图与设计稿比对
- [ ] 验证正常展示状态
- [ ] 验证边界情况（空内容、长文本折行等）
...
若任务已明确属于简单静态还原或局部 UI 调整，且 GitNexus 或代码上下文未显示较大影响面，后续运行项目、截图比对和业务功能回归只作为可选项；除非用户明确要求、改动已经影响业务交互，或 GitNexus 影响面足够大，否则交付中说明“仅做静态检查，未运行测试/smoke/运行态验证”。
