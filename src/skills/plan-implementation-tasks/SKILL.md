...
- 前端编码可以形成一个或多个并行组：根据接口约定文档编码，并按页面、组件、状态管理、接口适配、权限分支、空状态 / 异常状态、测试等边界拆成若干任务；只有写入范围不重叠且互不依赖的前端任务才可并行。
- 后端编码可以形成一个或多个并行组：根据接口约定文档编码，并按路由、服务、数据模型、迁移、队列 / worker、权限、错误语义、测试等边界拆成若干任务；只有写入范围不重叠且互不依赖的后端任务才可并行。
...
- 联调完成后必须串行安排前后端集成测试收口：覆盖主流程、失败路径、权限边界、重复提交、网络 / 存储不可用等完成标准；测试任务数量按功能域和验证成本拆分，不固定为一个任务。
...
- 对前后端项目，不能把“前端编码”和“后端编码”放在接口约定文档之前并行；也不能把联调或集成测试提前放入前后端编码并行组。
- 对前后端项目，不得机械生成固定 5 个任务；应先给出依赖骨架，再按实际功能域扩展前端任务、后端任务、联调任务和集成测试任务。
