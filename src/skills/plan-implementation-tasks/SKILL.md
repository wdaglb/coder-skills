...
4. 若发现任务文件与实际代码结构冲突，停止执行并回到主 agent 重新调整任务文件。
...
3. 再执行该并行组的统一验证命令。
4. 若整合失败，先定位是哪一个任务边界定义不足；必要时回到任务文件调整，而不是让 agent 继续盲改。

### 4. 串行推进下一组

1. 只有当前组完成标准全部满足后，才进入下一组。
2. 下一组开始前，重新确认依赖输入是否已经由上一组产出。
3. 若上一组产出与下一组预期不一致，停止并调整实施指导或任务文件。
...
3. 如果发现必须修改禁止范围、共享接口、迁移、配置或其他 agent 负责范围，立即停止并汇报。
4. 完成后运行任务文件指定验证，并在结果中列出修改文件、验证结果、未完成项和边界风险。
```
...
- 发现方案文件与代码现状不一致：停止实施，更新任务文件或实施指导后再继续。
