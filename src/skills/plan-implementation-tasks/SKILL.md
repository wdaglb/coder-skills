---
name: plan-implementation-tasks
description: 规划实施任务。Use when a confirmed plan file has already been written to docs/plans and the user wants to split implementation into executable task files under docs/tasks/{plan-file}/tasks-1.md, tasks-2.md, etc. Trigger after plan-file landing confirmation when the user chooses to拆分实施任务; this skill creates task files plus an implementation-guide.md that marks serial vs parallel groups, provides detailed main-agent and child-agent execution flow, handles boundary events, and recommends child-agent parallel execution for independent tasks, but must not implement the plan.
---

# Plan Implementation Tasks

## 概述

把已经确认并落地的方案文件拆成可执行任务文件。
本技能只负责从方案生成 `docs/tasks/<方案文件>/tasks-N.md` 与 `docs/tasks/<方案文件>/implementation-guide.md`，不负责修改业务代码、运行生成命令或开始实施。实施任务规划完毕后，必须询问用户是否按实施指导文件逐步执行。
拆分时必须把“编码任务的静态验证”和“最终测试 / 验收”分开：子智能体负责的编码任务只完成代码静态验证，最终自动化测试、集成测试、端到端测试、手工验收或回归测试必须交给独立测试任务串行收口。
进入最终测试前必须先使用 `$audit-completion-risks` 审查当前 diff 的真实风险；风险审查是最终测试的前置串行门禁，不得交给子智能体并行执行，也不得用最终测试替代风险审查。

## 工作流

### 1. 确认输入边界

使用本技能前必须已经满足：

- 方案已经写入 `docs/plans`。
- 用户已经明确选择要拆分实施任务。
- 当前动作仍处于任务规划阶段，不等于授权开始实施。

若任一条件不满足，停止并回到上游确认环节；不要自行补全用户授权。

### 2. 读取方案文件

读取方案文件前先遵循 `$large-file-gate`：

- 先检查文件大小。
- 文件大于 100KB 时，不得直接读取，必须先请求用户确认。
- 文件不大于 100KB 时，也只读取生成任务文件所需的最小内容。

若方案文件路径不明确，优先从最近一次写入 `docs/plans` 的结果、当前回复上下文或用户明确指定路径中确认；仍无法确定时，用固定选项要求用户选择，而不是猜测文件名。

### 3. 拆分任务并判断并行性

按方案文件中的实施步骤、阶段、模块边界或可验证交付物拆分任务。

拆分规则：

- 每个任务文件只对应一个实施步骤。
- 一个步骤应能被单独执行、验证和回退。
- 不要把多个无关模块塞进同一个任务文件。
- 不要把命名、格式、局部常量这类不会改变实施路径的小细节单独拆成任务。
- 若方案本身没有步骤，应先从方案目标、范围、风险和验证口径中归纳最少必要步骤。

任务顺序应满足依赖关系：基础结构、数据模型、接口契约、业务实现、UI/调用方、验证与收尾通常按先后排列，但以方案文件为准。

前后端项目的默认依赖骨架：

- 仅当方案明确涉及前端与后端共同实现时，使用本骨架判断串行 / 并行；其它项目类型不要套用此固定结构，应按真实依赖关系分析后再决定是否并行。
- 该骨架不是固定 5 个任务，也不是固定 5 个分组；实际任务数量必须根据方案范围、模块边界、文件重叠、接口数量、数据模型、页面数量、验证口径和回退粒度扩展或收缩。
- 接口约定文档必须先串行完成：明确接口路径、方法、请求参数、响应结构、错误码 / message、鉴权、状态流转、兼容性与前后端边界。前端和后端编码都依赖它。
- 前端编码可以形成一个或多个并行组：根据接口约定文档编码，并按页面、组件、状态管理、接口适配、权限分支、空状态 / 异常状态、静态验证等边界拆成若干任务；只有写入范围不重叠且互不依赖的前端任务才可并行。
- 后端编码可以形成一个或多个并行组：根据接口约定文档编码，并按路由、服务、数据模型、迁移、队列 / worker、权限、错误语义、静态验证等边界拆成若干任务；只有写入范围不重叠且互不依赖的后端任务才可并行。
- 前端并行组与后端并行组之间也不是固定两个任务；如果接口文档稳定，且前后端只共享该契约，可以把满足条件的前端任务与后端任务放入同一并行阶段；如果仍存在共享字段、状态或错误语义未决，必须先回到接口约定文档收敛。
- 前后端编码完成后必须串行安排联调收口：统一处理接口差异、错误 message、鉴权状态、空数据、异常返回和兼容性问题；联调任务可按功能域拆多个，但下游集成测试必须依赖对应联调完成。
- 联调完成后必须先串行安排 `$audit-completion-risks` 风险审查任务：读取当前 diff，审查回归、边界失败、无关改动和验证不足；审查未完成前不得进入最终测试。
- 风险审查完成并处理阻塞风险后，必须串行安排独立测试任务收口：覆盖主流程、失败路径、权限边界、重复提交、网络 / 存储不可用等完成标准；测试任务数量按功能域和验证成本拆分，不固定为一个任务。
- 最终测试不得塞进前端编码、后端编码或子智能体并行任务中；子智能体只做自己负责范围内的静态验证，例如类型检查、lint、格式 / 语法检查、静态审查等不执行完整业务流程的检查。
- 若方案已有接口约定文档，可以把“确认并补齐接口约定文档”作为前置串行任务；不得跳过接口契约确认后直接让前后端并行编码。
- 若前后端任一侧依赖对方尚未稳定的字段、状态或错误语义，必须回到接口约定文档先收敛，再继续后续并行组。

并行性判断规则：

- 任务修改的文件、模块、数据结构、接口契约互不重叠，且不依赖彼此产物时，可以标记为可并行。
- 多个任务分别修改不同文件或不同目录模块，且依赖性不大、静态验证口径可独立完成时，优先标记为可并行候选。
- 多个任务会修改同一文件、同一公共类型、同一接口、同一迁移或同一配置时，默认标记为串行。
- 某任务依赖前置任务输出，例如先定义接口再接入调用方，必须把依赖关系写清楚，并放入后续串行阶段。
- 只要任务执行中发现新的依赖、共享接口变更、写入范围重叠或静态验证结果互相影响，立即停止该并行组后续派发，回到主智能体重新协调。
- 若并行性证据不足，按串行处理，不要为了提高速度强行并行。
- 可并行任务应按“并行组”组织，同一并行组内满足进入条件的任务可以交给多个子智能体（sub-agent）同时执行；不同并行组之间仍由主智能体按顺序推进。
- 对前后端项目，不能把“前端编码”和“后端编码”放在接口约定文档之前并行；也不能把联调或最终测试提前放入前后端编码并行组。
- 对前后端项目，不得机械生成固定 5 个任务；应先给出依赖骨架，再按实际功能域扩展前端任务、后端任务、联调任务、最终测试前风险审查任务和独立测试任务。

子智能体并行进入条件：

- 用户已经在实施确认选项中明确允许“启用子智能体并行执行”。
- 同一并行组内至少有两个任务，且任务写入范围按文件、目录或模块边界清晰分离。
- 当前并行组的前置依赖已经完成；组内任务之间没有必须先后发生的接口、数据、配置或迁移依赖。
- 每个任务都有明确的允许修改范围、禁止修改范围、静态验证方式和停手条件。
- 主智能体可以自动管理子智能体：负责派发任务、监测完成度、等待同组结果、审查 diff、整合静态验证，并在上一组完成后再通知或启动下一组；用户不需要手动管理主智能体与子智能体。
- 若任一子智能体报告依赖缺失、越界需求、共享文件冲突或静态验证失败，主智能体必须暂停该并行组整合，不再继续跨子智能体派发下游任务。

### 4. 写入任务文件与实施指导文件

输出目录规则：

- 若方案文件为 `docs/plans/<name>.md`，任务目录为 `docs/tasks/<name>/`。
- 若方案文件名包含空格或不适合目录名的字符，使用短横线连接的小写目录名。
- 文件命名固定为 `tasks-1.md`、`tasks-2.md`、`tasks-3.md`，按执行顺序递增。
- 同目录必须额外创建 `implementation-guide.md`，用于说明任务依赖、串行/并行分组和子智能体并行执行建议。

若目标任务目录已存在：

- 先读取目录清单，确认是否已有任务文件。
- 不要静默覆盖已有 `tasks-N.md` 或 `implementation-guide.md`。
- 用固定选项让用户确认覆盖、追加新序号或改目录。

### 5. 任务文件模板

每个任务文件使用下面结构：

```markdown
# 任务 N：<任务标题>

## 来源方案

- 方案文件：`docs/plans/<name>.md`
- 对应方案段落：<章节名或步骤名>

## 目标

- <本步骤要完成的可验证结果>

## 实施范围

- <需要修改或新增的模块、文件或接口>

## 依赖关系

- 前置任务：<无 / tasks-X.md>
- 可并行任务：<无 / tasks-Y.md>
- 并行限制：<为什么可以或不能并行>

## 执行步骤

1. <第一步>
2. <第二步>

## 验证方式

- <编码任务只写静态检查；最终自动化测试、集成测试、端到端测试、手工验证或回归测试必须拆到独立测试任务>

## 风险与回退

- 风险：<本步骤主要风险>
- 回退：<如何撤销或降级>

## 完成标准

- <判断本任务完成的具体标准>
```

如果某一项无法从方案文件中可靠得出，写明“待实施前确认”，不要编造细节。
编码任务如果可能由子智能体执行，`验证方式` 只能填写静态验证，例如类型检查、lint、格式检查、语法检查、静态审查或不启动完整业务流程的构建检查；不要填写需要真实服务、数据库、浏览器、外部依赖或跨模块运行态联调的最终测试命令。最终测试必须单独生成测试任务，并在依赖图中放到对应编码 / 联调任务之后。
最终测试任务之前必须单独生成 `$audit-completion-risks` 风险审查任务，或在最终测试任务的前置依赖中明确要求先完成该技能审查；若审查发现高 / 中风险，需要先处理或让用户确认后再进入最终测试。

### 6. 实施指导文件模板

`implementation-guide.md` 使用下面结构；内容必须足够细，能指导后续主智能体与子智能体按步骤执行，并在边界事件出现时停止或回到主智能体协调：

```markdown
# 实施指导：<方案标题>

## 来源方案

- 方案文件：`docs/plans/<name>.md`
- 任务目录：`docs/tasks/<name>/`

## 执行总览

- 总任务数：<N>
- 推荐执行方式：<串行 / 部分并行 / 全部并行>
- 子智能体建议：<是否建议启用子智能体并行执行>
- 并行进入条件：<哪些任务满足多个文件或模块改动、依赖性不大、写入范围不重叠、静态验证可独立完成>

## 任务依赖图

- tasks-1.md：<无前置 / 依赖 tasks-X.md>
- tasks-2.md：<无前置 / 依赖 tasks-X.md>
- 最终测试任务：<必须依赖 `$audit-completion-risks` 风险审查任务完成>

## 执行分组

### 第 1 组：<串行 / 并行>

- 执行任务：`tasks-1.md`, `tasks-2.md`
- 并行判断：<为什么能并行，或为什么必须串行>
- 子智能体建议：
  - 子智能体 1：负责 `tasks-1.md`，写入范围 `<文件或模块>`
  - 子智能体 2：负责 `tasks-2.md`，写入范围 `<文件或模块>`

## 详细执行流程

### 0. 执行前检查

1. 确认当前工作树状态，记录已有未提交改动。
2. 重新读取本指导文件和本组任务文件，确认任务范围、写入边界和前置依赖。
3. 若任务依赖上一组输出，先确认上一组已完成并通过对应静态验证；最终测试只在独立测试任务中执行。
4. 进入实施后，`docs/plans` 与 `docs/tasks` 下的方案、任务和实施指导文件默认只读；除非用户明确要求修改这些文件，否则不得编辑。
5. 若发现任务文件与实际代码结构冲突，停止执行并回到主智能体协调；只有用户明确要求修改方案或任务文件时，才允许更新对应文件。

### 1. 派发并行组

1. 主智能体先确认用户已选择“启用子智能体并行执行”；若用户只选择串行执行，不得启动子智能体。
2. 主智能体只把同一并行组内、写入范围不重叠且依赖性不大的任务派发给多个子智能体。
3. 每个子智能体的任务说明必须包含：任务文件、允许修改范围、禁止修改范围、静态验证命令、边界事件处理规则。
4. 子智能体启动后，主智能体不再同时修改这些子智能体的负责范围，避免制造冲突。
5. 主智能体负责自动监测子智能体完成度；同一并行组未全部完成前，不通知或启动依赖该组产出的下游任务。
6. 任一子智能体报告越界需求、依赖缺失或冲突时，主智能体暂停该并行组后续整合。

### 2. 执行单个任务

1. 先读取任务文件中的目标、实施范围、依赖关系和完成标准。
2. 只修改任务文件列出的文件、模块或接口范围。
3. 按任务文件的执行步骤逐项实施；每完成一个关键步骤，确认没有越过写入边界。
4. 编码任务只执行任务文件列出的静态验证；若静态验证失败，先在本任务范围内修复。
5. 若任务文件要求执行最终自动化测试、集成测试、端到端测试、手工验收或回归测试，停止并回到主智能体协调，不能由子智能体自行扩大验证范围。
6. 若修复需要超出本任务范围，停止并向主智能体汇报，不得自行扩大范围。

### 3. 并行组整合

1. 等同一并行组内所有子智能体完成后，主智能体统一检查 diff。
2. 先检查是否存在文件重叠、接口不一致、迁移顺序冲突、配置互相覆盖。
3. 再执行该并行组的统一静态验证命令，不在并行组整合阶段执行最终测试。
4. 若整合失败，先定位是哪一个任务边界定义不足；默认只调整实施动作或代码范围，不修改已落地的任务文件；确需修改方案或任务文件时，必须先取得用户明确确认。

### 4. 串行推进下一组

1. 只有当前组完成标准全部满足后，才进入下一组。
2. 下一组开始前，重新确认依赖输入是否已经由上一组产出。
3. 若上一组产出与下一组预期不一致，停止并回到主智能体协调；不得默认调整实施指导或任务文件，除非用户明确要求修改。
4. 进入最终测试任务前，主智能体必须先使用 `$audit-completion-risks` 审查当前 diff；审查发现阻塞风险时，先处理风险或回到用户确认，不得直接开始最终测试。

## 子智能体执行约束

- 只有同一并行组内、写入范围不重叠且依赖性不大的任务可以并行交给多个子智能体。
- 每个子智能体必须只修改自己负责的任务文件对应范围。
- 方案文件、任务文件和 `implementation-guide.md` 是执行输入，不是实施输出；开始实施后对所有子智能体都是只读。
- 每个子智能体都必须知道当前不是独占代码库，不能回退、覆盖或整理其他智能体的改动。
- 若执行中发现需要修改其他子智能体的负责范围，必须停止并回到主智能体重新协调。
- 并行组完成后，主智能体负责统一检查 diff、整合冲突并执行静态验证；最终测试交给独立测试任务。
- 主智能体负责跨子智能体的自动编排：派发、等待、汇总、通知下一组、处理冲突，不要求用户手动协调子智能体。

## 子智能体派发模板

```text
你负责执行 `<tasks-N.md>`。

允许修改范围：
- <明确列出文件、目录或模块>

禁止修改范围：
- <明确列出不能触碰的文件、目录、接口或配置>
- `docs/plans/**`
- `docs/tasks/**`

执行要求：
1. 先读取任务文件，确认目标、依赖、执行步骤和完成标准。
2. 只在允许修改范围内实施，不要整理、回退或覆盖其他智能体的改动。
3. 不得修改方案文件、任务文件或实施指导文件；如果用户明确要求修改，必须由主智能体重新协调后再处理。
4. 如果发现必须修改禁止范围、共享接口、迁移、配置或其他子智能体负责范围，立即停止并汇报。
5. 完成后只运行任务文件指定的静态验证，并在结果中列出修改文件、静态验证结果、未完成项和边界风险。
6. 不得执行最终自动化测试、集成测试、端到端测试、手工验收或回归测试；如果任务文件要求这些验证，立即停止并汇报给主智能体。
```

## 边界事件处理

本节不能只写“按情况处理”，必须至少覆盖下面这些边界事件；如果某一项看起来不适用，也要写明“不适用原因”：

- 发现任务需要修改未授权文件：停止该任务，回到主智能体重新协调。
- 发现两个子智能体需要修改同一文件：取消并行，改为串行或重新拆分任务。
- 发现共享接口、类型、迁移或配置需要调整：先单独形成前置任务，完成后再继续下游任务。
- 发现方案文件与代码现状不一致：停止实施，向用户说明冲突；除非用户明确要求修改方案或任务文件，否则保持 `docs/plans` 与 `docs/tasks` 只读。
- 发现静态验证命令缺失或不可运行：记录原因，先补充可替代静态验证方式；若验证口径会影响完成标准，回到用户确认。
- 发现子智能体任务包含最终测试：停止派发或召回该任务，拆出独立测试任务后再继续；不要让子智能体用最终测试替代静态验证。
- 发现最终测试前未执行 `$audit-completion-risks`：停止最终测试，先补做风险审查；若审查发现阻塞风险，处理或确认后再恢复最终测试。
- 发现执行会扩大到方案外功能：停止并要求用户确认范围，不能把范围扩散包装成实现细节。
- 发现子智能体未完成但下游任务想继续：主智能体必须等待或停止下游派发，不得让跨子智能体依赖任务提前执行。
- 发现并行结果发生冲突：主智能体负责整合；必要时回滚冲突任务的局部改动并改为串行重做，不能让多个子智能体同时抢修同一冲突。
- 发现存在用户未确认的破坏性动作、数据迁移、权限变更或高风险 Git 操作：停止并转回对应门禁确认。

## 推荐实施顺序

1. <先执行哪一组>
2. <再执行哪一组>

## 验证与收口

- <并行组完成后的统一静态验证方式>
- <进入最终测试前的 `$audit-completion-risks` 风险审查方式>
- <独立测试任务中的最终测试 / 验收方式>
```

若所有任务都必须串行，仍然创建 `implementation-guide.md`，并明确说明“不建议启用子智能体并行执行”的原因。

## 实施后的只读规则

当用户确认按 `implementation-guide.md` 执行后：

- `docs/plans/**`、`docs/tasks/**`、`implementation-guide.md` 都视为已确认输入，默认只读。
- 实施过程中只能读取这些文件来对照范围、步骤、验证方式和停手条件。
- 不得把“任务边界需要调整”“代码现状不一致”“验证口径不完整”直接转化为修改方案或任务文件。
- 若确实需要修改方案、任务文件或实施指导文件，必须先停止实施，并用固定选项让用户确认是否修改这些文件。
- 用户没有明确要求修改时，优先通过调整实施动作、补充代码内处理或回到主智能体协调来解决，不能改写已确认计划。

### 7. 交付后询问是否按实施指导执行

任务文件全部写入后：

- 汇报创建了哪些文件。
- 汇报 `implementation-guide.md` 中的推荐执行方式：串行、部分并行或全部并行。
- 若存在可并行组，说明是否建议启用子智能体并行执行，以及进入条件是什么。
- 说明尚未开始实施。
- 必须继续用固定选项询问用户是否按实施指导文件逐步执行；选项 1 / 2 / 3 / 4 必须使用下面口径，不要改写成“开始实施”“开始逐组实施”等模糊说法：

```text
【选择】
1. 按 implementation-guide.md 执行，并对满足并行进入条件的任务启用子智能体并行执行
2. 按 implementation-guide.md 执行，但全部由当前主智能体串行执行
3. 先检查并调整任务文件或实施指导，暂不实施
4. 用户自行填写
```

用户未明确选择前，不得开始执行任何任务文件中的实施步骤。

## 硬约束

- 不把“拆分任务”理解为“开始实施”。
- 不在任务文件写入后直接修改业务代码。
- 不在未确认覆盖策略时覆盖已有任务文件或实施指导文件。
- 不把当前推荐拆分方案写成用户已确认事实。
- 不跳过 `docs/tasks/<方案文件>/tasks-N.md` 的固定落点。
- 不跳过 `implementation-guide.md`；即使所有任务都必须串行，也要写清楚串行原因。
- 不在实施任务规划完毕后直接开始执行；必须先询问用户是否按 `implementation-guide.md` 按步骤执行。
- 不把存在共享文件、共享接口、迁移顺序或上下游依赖的任务标记为可并行。
- 不让多个子智能体同时修改同一文件、同一公共契约或同一持久化结构。
- 不在用户未明确选择“启用子智能体并行执行”时启动子智能体。
- 不要求用户手动管理主智能体与子智能体；主智能体必须自动负责子智能体派发、完成度监测、整合与下游通知。
- 不生成只有任务列表、没有详细执行流程和边界事件处理的 `implementation-guide.md`。
- 不向子智能体派发缺少允许修改范围、禁止修改范围、静态验证方式和停手条件的任务说明。
- 不让子智能体执行最终自动化测试、集成测试、端到端测试、手工验收或回归测试；最终测试必须由独立测试任务串行收口。
- 不跳过最终测试前的 `$audit-completion-risks` 风险审查；该审查必须在最终测试任务前串行完成，且不得交给子智能体并行执行。
- 不省略边界事件处理清单中的任一必备边界；不适用时也必须说明不适用原因。
- 不把最终确认选项 1 改写成“开始实施”或“开始逐组实施”；必须明确是“按 implementation-guide.md 执行，并对满足并行进入条件的任务启用子智能体并行执行”。
- 不在开始实施后修改 `docs/plans/**`、`docs/tasks/**` 或 `implementation-guide.md`；除非用户明确要求修改这些文件。
